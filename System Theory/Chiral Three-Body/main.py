...
class ThreeBody(Swarmalators2D):
    ...
    def _pointTheta(phaseTheta: np.ndarray, other1: np.ndarray, other2: np.ndarray,
        ...
        n, m, l = phaseTheta.shape[0], other1.shape[1], other2.shape[2]
        k1 = np.zeros((n,), dtype=np.float64)
        for i in range(n):
            for j in range(m):
                for k in range(l):
                    k1[i] += (
                            strengthLambda1 * K1[i, j, k] * np.sin(other1[i, j, k] - phaseTheta[i, j, k]) +
                            strengthLambda2 * K2[i, j, k] * np.sin(
                        other1[i, j, k] + other2[i, j, k] - 2 * phaseTheta[i, j, k])
                    )
